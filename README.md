...
You have a microservice that is deployed as an OpenWhisk HTTP action or even a number of these microservices and you want to establish monitoring of service uptime using New Relic.
...
Finally, you know that there are monitoring services [New Relic Synthetics](https://docs.newrelic.com/docs/synthetics) but you do not want to keep repeating the same code for returning a status check in each of your micro services.
...
All `GET /_status_check/healthcheck.json` requests to your service will now respond with a json response.
...
[New Relic Synthetics](https://docs.newrelic.com/docs/synthetics) can be used with `helix-status` by creating an API Check script like this:
...
```javascript
module.exports.main = wrap(main, 
  { example: (params) => params.foo === params.bar });
```
